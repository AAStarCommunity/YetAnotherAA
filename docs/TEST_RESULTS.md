...
| **OPR-01** 完整准入（registerRole→deploy xPNTs→paymaster→deposit） | ✅正向 | 🟡 **已建，推进到 register，fixme** | `operator-onboarding.spec.ts`。**进展**：换新 Infura RPC + `withRetry`(receipt 等待) + 注入钱包 sendTx 显式 gas(25/3) 后，**注资 → connect → AOA → 资源预检全部通过**，推进到首个写步骤 **Register Community**。**blocker = registry 合约 revert（非测试代码）**：Step3 `registryActions.registerRole(ROLE_COMMUNITY)` 链上 revert。已用 `simulateContract` **定论**：充足的 fresh EOA（70 GT + 对 GTokenStaking 和 Registry 各 30 allowance）仍**裸 revert "execution reverted"**（无 reason、无具名 error）→ 排除 allowance/spender/余额；而已注册 EOA 同调用能干净解码为 `RoleAlreadyGranted`（证明 ABI/编码对）。深挖发现：Step3 用的是 `registerRole(roleId, **user**, data)`（管理员版），SDK 自己的 OperatorClient 流程用 `registerRoleSelf(roleId, data)`（自助版）——但 **registerRoleSelf 对同一充足 fresh EOA 也裸 revert**，所以不止入口问题，还有 registry 未暴露的前置。**已提 SDK issue [AAStarCommunity/aastar-sdk#169](https://github.com/AAStarCommunity/aastar-sdk/issues/169)**（求正确社区注册流程 + 加 revert reason + 导出缺失的 registerRoleSelf）。外部仓库 → 阻塞在该回复，不在此 patch。GToken 已 mint 补足（主 EOA 是 owner）。标 `test.fixme` |
